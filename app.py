...
# --- 页面配置 ---
st.set_page_config(page_title="色彩管理 Pro (1x8 竖排大字版)", layout="wide")
st.title("🎨 色彩提取工具 (1行8个 + 竖排超大字)")
...
def create_tif_chart(selected_items, mode="RGB"):
    ...
    COLUMNS = 8 
    MARGIN_PX = int(0.6 * CM_TO_INCH * DPI) 
    # 竖排文字需要更多垂直空间
    TEXT_H_PX = int(2.5 * CM_TO_INCH * DPI) 
    ...
    if mode == "RGB":
        ...
        bg_color, text_color = (255, 255, 255, 255), (0, 0, 0, 255) 
    ...
    for i, item in enumerate(selected_items):
        ...
        # 2. 绘制标注 (spacing 控制行间距)
        draw.text((x, y + BLOCK_PX + 20), label, fill=text_color, font=font, spacing=15)
...
if 'data_list' in st.session_state:
    ...
    for i, row in enumerate(st.session_state['data_list']):
        ...
        with col_txt:
            st.write(f"颜色 {i+1}")
        with col_pre1:
            st.markdown(f'<div style="background-color:{row["设计图色块"]}; height:40px; border:1px solid #000;"></div>', unsafe_allow_html=True)
        with col_pre2:
            st.markdown(f'<div style="background-color:{row["工厂稿色块"]}; height:40px; border:1px solid #000;"></div>', unsafe_allow_html=True)
        if checked: selected_indices.append(row)

    if selected_indices:
        ca, cb = st.columns(2)
        with ca:
            st.download_button("📥 下载 RGB 校色条 (1x8 竖排大字)", create_tif_chart(selected_indices, "RGB"), "RGB_1x8.tif", "image/tiff")
        with cb:
            st.download_button("📥 下载 CMYK 打样条 (1x8 竖排大字)", create_tif_chart(selected_indices, "CMYK"), "CMYK_1x8.tif", "image/tiff")
